...
## Оценка точности и ошибок

В текущем прототипе точность прогнозов не измеряется в реальном времени, потому что нет механизма автоматической сверки с фактическим движением цены.

Однако архитектура это позволяет: каждый прогноз сохраняется в таблицу Prediction с указанием времени создания. Для оценки точности достаточно сравнить направление прогноза с фактическим изменением цены спустя 24 часа (или другой выбранный период). Это можно сделать вручную или написать отдельный скрипт, который соберёт историю прогнозов и сверится с историческими ценами CoinGecko.

Такой подход позволит понять, ошибается ли система, и при необходимости скорректировать веса или добавить новые сигналы.
